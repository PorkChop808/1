...
def bot_loop():
    ...
    with mss.mss() as sct:
        mon2 = sct.monitors[2]  # Monitor 2
        # Capture the center half of monitor 2
        half_width = mon2['width'] // 3
        left = mon2['left'] + (mon2['width'] - half_width) // 3
        region = {
            "left": left,
            "top": mon2['top'],
            "width": half_width,
        ...
        while bot_running:
            ...
            if detected_classes:
                ...
                if 'wild' in detected_classes and (now - last_cast_time) >= 2:
                ...
                if elapsed >= 1 and 'calm' not in detected_classes:
                    pyautogui.mouseDown(button='left')
                    time.sleep(random.uniform(0.05, .5))  # Add randomness to hold time
                    pyautogui.mouseUp(button='left')
                    holding_left = False
                    last_cast_time = time.time()
                    no_detect_start = time.time() + 4  # Wait 5 seconds total after cast (1s hold + 4s wait)
